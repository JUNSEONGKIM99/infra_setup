...
#	@docker build ./mlflow
	...
	@docker build ./dcgm-exporter --no-cache --tag aibiz_dcgm-exporter:4.4.0
...
# 	@docker build ./tomcat --no-cache --tag aibiz_tomcat:9.0.69-jdk17
	@mkdir -p ./volumes/springboot/app/config
	@mkdir -p ./volumes/springboot/app/workspace
	@mkdir -p ./volumes/springboot/app/logs
...
#	@docker build ./coredns --no-cache --tag aibiz_coredns:1.12.0
	...
	@mkdir -p ./volumes/kafka ./volumes/kafka/data
	@docker build ./kafka --no-cache --tag aibiz_kafka:4.0.0

	@mkdir -p ./volumes/flink ./volumes/flink/data
	@docker build ./flink --no-cache --tag aibiz_flink:2.0.0
	@wget https://repo1.maven.org/maven2/org/apache/flink/flink-sql-connector-kafka/4.0.0-2.0/flink-sql-connector-kafka-4.0.0-2.0.jar -O ./flink/jars/flink-sql-connector-kafka-4.0.0-2.0.jar

	@mkdir -p /data/volumes/vllm-data/hf-cache /data/volumes/vllm-data/lmcache
	@docker build ./vll-sub1 --no-cache --tag my-vllm:0.9.9

	@mkdir -p ./volumes/open-webui
	@docker build ./open-webui --no-cache --tag my-webui
...
.PHONY: semes-up
semes-up: create-network
	$(call docker_up,semes)
...
.PHONY: dash2-up
dash2-up: create-network
	$(call docker_up,dash2)

.PHONY: dash2-down
dash2-down:
	$(call docker_down,dash2)
...
# -----------------------------------------------------------------------------
.PHONY: kafka-up
kafka-up: create-network
	$(call docker_up,kafka)

# 서비스 실행(flink)
# -----------------------------------------------------------------------------
.PHONY: flink-up
flink-up: create-network
	$(call docker_up,flink)

# 서비스 실행(fastapi)
.PHONY: fastapi-up
fastapi-up: create-network
	$(call docker_up,fastapi)

# 서비스 실행(vllm)
# -----------------------------------------------------------------------------
.PHONY: vllm-up
vllm-up: create-network
	$(call docker_up,vllm)

# 서비스 실행(open-webui)
# -----------------------------------------------------------------------------
.PHONY: open-webui-up
open-webui-up: create-network
	$(call docker_up,open-webui)
...
.PHONY: kafka-down
kafka-down:
	$(call docker_down,kafka)

# 서비스 종료(flink)
# -----------------------------------------------------------------------------
.PHONY: flink-down
flink-down:
	$(call docker_down,flink)

# 서비스 종료(fastapi)
# -----------------------------------------------------------------------------
.PHONY: fastapi-down
fastapi-down:
	$(call docker_down,fastapi)

# 서비스 종료(vllm)
# -----------------------------------------------------------------------------
.PHONY: vllm-down
vllm-down:
	$(call docker_down,vllm)

# 서비스 종료(open-webui)
# -----------------------------------------------------------------------------
.PHONY: open-webui-down
open-webui-down:
	$(call docker_down,open-webui)
...
help:
	...
	@echo "  applied service list   nginx, certbot, minio, postgres, trino, spark, airflow, mlflow, sqlmesh, portainer, jupyterhub, grafana, keycloak, nextcloud, vllm, open-webui"
	@echo ""
